...
## 1. Select 문에서의 연산
- `SELECT`문은 기본적으로 DB에 있는 데이터 특정 열을 호출하는 기능
- 동시에 (1)반복 호출해서 열을 중복으로 구성하는 것도 가능할 뿐더러, (2)함수나 연산하여 가공된 열을 호출하는 것도 가능
...
FROM
    테이블
```
- 만약 `Dollar` 값을 한화로 (1,100원) 보려면 아래와 같이 연산을 열에 추가하여 가능
```sql
...
FROM
    테이블
```
- 위의 예제들을 보면, SQL의 `SELECT`문은 하나의 열 단위로 프로세스가 반복해서 이루어짐을 확인할 수 있음
___
...
FROM
    테이블
```
- 외, 날짜 데이터도 년(`YEAR`), 월(`MONTH`), 일(`WEEKDAY`) 단위로 추출할 수 있음
